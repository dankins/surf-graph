...
class GraphQueryModuleSpec extends Specification with NoTimeConversions {
  ...
  "GraphQueryModule" should {
    "allow retrieval of a vertex" in new TestContext {
      val v = Await.result(graphQuery.get("1".asInstanceOf[graphQuery.idType]), 30 seconds)
      v.id must be equalTo "1".asInstanceOf[graphQuery.idType]
    }
    ...
    "get a segment" in new TestContext {
      val segment = Await.result(graphQuery.getSegment("7".asInstanceOf[graphQuery.idType]), 30 seconds)
      segment.v1.id must be equalTo "1".asInstanceOf[graphQuery.idType]
      segment.v2.id must be equalTo "2".asInstanceOf[graphQuery.idType]
    }
    "get a segment from vertices" in new TestContext {
      val segment = Await.result(graphQuery.getSegmentFromVertices("1".asInstanceOf[graphQuery.idType],"2".asInstanceOf[graphQuery.idType],"knows"), 30 seconds)
    ...
    "get edges" in new TestContext {
      val edges = Await.result(graphQuery.getEdges("1".asInstanceOf[graphQuery.idType],Direction.OUT,"knows"), 30 seconds)

      edges.map(_.edge.label) must be equalTo Seq("knows","knows")
      edges.map(_.vertex.id) must be equalTo Seq("2".asInstanceOf[graphQuery.idType],"4".asInstanceOf[graphQuery.idType])
    }
